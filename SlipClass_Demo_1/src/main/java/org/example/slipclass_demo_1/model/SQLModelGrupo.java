...
public class SQLModelGrupo {
    ...
    public static int verificarCodigoGrupo(String codGrupo) {
        String sql = "SELECT Id_Grupo FROM GRUPO WHERE codGrupo = ? AND (fecha_eliminacion IS NULL OR Id_Estado != 4)";
        ...
        try (Connection conn = SQLDataAccess.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return rs.getInt("Id_Grupo"); // Retorna el ID si el grupo existe y está activo
                }
            }
        } catch (SQLException e) {
            System.err.println("Error al verificar código de grupo: " + e.getMessage());
        }
        return -1; // Retorna -1 si el grupo no existe
    }
    }

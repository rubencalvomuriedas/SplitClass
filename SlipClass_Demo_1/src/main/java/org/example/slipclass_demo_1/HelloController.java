...
public class HelloController implements Initializable {
    ...
    void onAnadirGrupooClick(ActionEvent event) {
        ...
        if (usuarioActual == null) {
            mostrarAlerta("Error de Sesión", "No hay ningún usuario activo.", Alert.AlertType.ERROR);
            return;
        }

        TextInputDialog dialog = new TextInputDialog();
        dialog.setTitle("Unirse a un Grupo");
        dialog.setHeaderText("Introduce el código único del grupo");
        dialog.setContentText("Código de Grupo:");

        Optional<String> result = dialog.showAndWait();

        result.ifPresent(codigo -> {
            String codigoLimpio = codigo.trim().toUpperCase(); // Limpiamos espacios y pasamos a mayúsculas

            if (codigoLimpio.isEmpty()) {
                mostrarAlerta("Campo Vacío", "Debes introducir un código válido.", Alert.AlertType.WARNING);
                return;
            }

            int idGrupoDestino = SQLModelGrupo.verificarCodigoGrupo(codigoLimpio);

            if (idGrupoDestino != -1) {

                boolean exito = SQLModelGrupo.agregarMiembroAlGrupo(usuarioActual.getId_usuario(), idGrupoDestino);

                if (exito) {
                    mostrarAlerta("¡Éxito!", "Te has unido al grupo correctamente.", Alert.AlertType.INFORMATION);



                } else {
                    mostrarAlerta("Aviso", "No se pudo procesar. Es posible que ya formes parte de este grupo.", Alert.AlertType.WARNING);
                }

            } else {
                mostrarAlerta("Código Incorrecto", "El código introducido no corresponde a ningún grupo activo.", Alert.AlertType.ERROR);
            }
        });
    }
